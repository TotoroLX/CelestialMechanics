...
datFiles1 = dir("x0po_T_from_2_*.dat");
N1 = length(datFiles1);
x0po1 = [];
for i=1:N1
...
errs = zeros(size(x0po_iter, 1), 3);

figure()
hold on
grid on
for k = 1:1:size(x0po_iter,1)
    x0 = x0po_iter(k,1:6) ;
    tf = x0po_iter(k, end) ;
    [x,t] = trajGet3BP3d(x0,tf,mu,OPTIONS) ;
    
    errs(k) = max( abs( x0(1:3) - x(end,1:3) ) ) ;
...
figure()
plot(x0po_iter(1:end, 3), Jc(1:end), 'bo', 'MarkerSize', 2)
